...
class Smarty_Template_Compiled extends Smarty_Template_Resource_Base {
	...
	public function populateCompiledFilepath(Smarty_Internal_Template $_template) {
		...
		if (isset ( $_template->compile_id )) {
			$this->filepath .= preg_replace ( '![^\w]+!', '_', $_template->compile_id ) . ($smarty->use_sub_dirs ? $smarty->ds : '^');
		}
		// if use_sub_dirs, break file into directories
		if ($smarty->use_sub_dirs) {
			$this->filepath .= $source->uid [0] . $source->uid [1] . $smarty->ds . $source->uid [2] . $source->uid [3] . $smarty->ds . $source->uid [4] . $source->uid [5] . $smarty->ds;
		}
	...
	public function process(Smarty_Internal_Template $_smarty_tpl) {
		...
		if ($source->handler->recompiled) {
			...
			if (! $this->exists || $smarty->force_compile || ($smarty->compile_check && $source->getTimeStamp () > $this->getTimeStamp ())) {
